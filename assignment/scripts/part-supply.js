...
for (i = 0; i < supplyChanges.length; i++) {
        let thisSupply = supplyChanges[i]

        if (thisSupply > 0){
            console.log('Added '+ thisSupply +' parts.' );
        }
         if (thisSupply === 0){
            console.log('No Change');
         }
         if (thisSupply < 0){
            console.log('Removed '+ thisSupply +' parts.');
         }
        }
...
let supplyChanges = [3,5,-6,0,7,11];
		...
		for (let i = 0; i < supplyChanges.length; i++)
			(inventory += supplyChanges[i]);

console.log("Total parts available = " + (inventory));
}
